...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root { --ink: #0f0e0c; --paper: #f7f4ee; --accent: #c8501a; --accent-light: #f0d5c8; --muted: #7a7570; --border: #d6d0c6; --card: #ffffff; --green: #2d7a2d; --green-bg: #eef6ee; }
    ...
    header { border-bottom: 1px solid var(--border); padding: 0 40px; display: flex; align-items: center; justify-content: space-between; height: 64px; background: rgba(247,244,238,0.92); backdrop-filter: blur(10px); position: sticky; top: 0; z-index: 100; animation: slideDown 0.5s ease; }
    ...
    .upload-zone { border: 2px dashed var(--border); border-radius: 12px; padding: 50px 24px; text-align: center; transition: all 0.22s ease; cursor: pointer; position: relative; background: var(--paper); }
    ...
    @media (max-width: 600px) { header { padding: 0 20px; } .card { padding: 22px 18px; } .tips { grid-template-columns: 1fr; } .hero { padding: 44px 20px 28px; } }
  </style>
</head>
<body>
<header>
  <div class="logo"><span class="logo-dot"></span>NoteScribe<span class="by-name">by Gowrishankar</span></div>
...
<div class="container">
  ...
  <div class="card">
    <form method="POST" enctype="multipart/form-data" action="/gowrishankar/convert" id="upload-form">
      ...
      <div class="tips">
        <div class="tip"><div class="tip-label">Lighting</div><p>Shoot in natural daylight, avoid shadows.</p></div>
        <div class="tip"><div class="tip-label">Angle</div><p>Hold phone directly above, flat and parallel.</p></div>
        <div class="tip"><div class="tip-label">Pen</div><p>Dark ink on white paper gives best results.</p></div>
      </div>
...
<footer>NoteScribe &mdash; Built by <strong>Gowrishankar</strong> &mdash; Powered by Google Vision AI</footer>
<script>
  function handleFile(input) {
    const file = input.files[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = e => { document.getElementById("preview-img").src = e.target.result; document.getElementById("file-label").textContent = file.name; document.getElementById("preview-wrap").style.display = "block"; };
    reader.readAsDataURL(file);
  }
  const dz = document.getElementById("drop-zone");
  dz.addEventListener("dragover", e => { e.preventDefault(); dz.classList.add("dragover"); });
  dz.addEventListener("dragleave", () => dz.classList.remove("dragover"));
  dz.addEventListener("drop", e => { e.preventDefault(); dz.classList.remove("dragover"); const f = e.dataTransfer.files; if (f.length) { document.getElementById("file-input").files = f; handleFile(document.getElementById("file-input")); } });
  document.getElementById("upload-form").addEventListener("submit", function(e) {
    const fi = document.getElementById("file-input");
    if (!fi.files.length) { e.preventDefault(); alert("Please select an image first."); return; }
    const btn = document.getElementById("submit-btn");
    btn.innerHTML = "<span class='spinner'></span> Converting..."; btn.disabled = true; btn.classList.add("loading");
  });
</script>
</body>
</html>
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .logo { font-family: Georgia, serif; font-size: 1.35rem; letter-spacing: -0.02em; color: var(--ink); display: flex; align-items: center; gap: 10px; }
    .logo-dot { width: 8px; height: 8px; background: var(--accent); border-radius: 50%; animation: pulse 2.5s infinite; }
    .by-name { font-size: 0.72rem; font-weight: 500; color: var(--muted); }
    .badge { font-size: 0.7rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--accent); border: 1px solid var(--accent-light); background: var(--accent-light); padding: 3px 11px; border-radius: 20px; }
    ...
    .btn-copy { display: inline-flex; align-items: center; gap: 8px; background: transparent; color: var(--ink); border: 1px solid var(--border); padding: 11px 20px; font-size: 0.88rem; border-radius: 8px; cursor: pointer; transition: all 0.2s ease; font-family: "Segoe UI", Ubuntu, sans-serif; }
    ...
    footer { text-align: center; padding: 28px; font-size: 0.76rem; color: var(--muted); border-top: 1px solid var(--border); }
    footer strong { color: var(--accent); font-weight: 600; }
    @media (max-width: 600px) { header { padding: 0 20px; } .container { padding: 24px 18px 60px; } .word-count { margin-left: 0; } }
  </style>
</head>
<body>
<header>
...
<script>
  function copyText() {
    ...
    navigator.clipboard.writeText(text).then(() => { const l = document.getElementById("copy-label"); l.textContent = "Copied!"; setTimeout(() => l.textContent = "Copy text", 2000); });
  }
  const o = document.getElementById("output-text");
  if (o) { const w = o.innerText.trim().split(/\s+/).filter(Boolean).length; document.getElementById("word-count").textContent = w + " words"; }
</script>
</body>
</html>
'''
